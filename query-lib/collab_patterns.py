"""
Cypher queries for extracting collaboration-pattern occurrences from an
Event Knowledge Graph (EKG) for adaptive multi-robot mission execution.

The module follows the data model used in the paper draft:

Nodes
-----
(:Event {event_id, activity, start, end})
(:Entity {id, type})                    # type in {'Mission', 'Segment', 'Robot'}
(:Capability {id/name/...})

Relationships
-------------
(:Event)-[:CORR]->(:Entity)
(:Event)-[:DF {perspective_id, perspective_type, transitionTimeSeconds}]->(:Event)
...
Notes
-----
1. Pattern occurrences are implemented as query-defined views. The queries return
   tuples corresponding to the occurrence sets I_p(G); they do not require adding
   PatternOccurrence nodes to the EKG.
2. DF edges should be materialized per perspective. For example, a DF edge over
   mission m1 should have perspective_type='Mission' and perspective_id='m1'.
3. transitionTimeSeconds is expected on DF edges. If unavailable, occurrence
   queries fall back to duration.inSeconds(e_i.end, e_j.start).seconds.
4. Event start/end are assumed to be Neo4j datetime values, as produced by the
   companion loader. If your graph stores numeric timestamps, use the numeric
   annotation query and adapt the duration expressions accordingly.
5. The property names used below are centralized in Schema so they can be adapted
   to a concrete Neo4j database.
"""
...
class Schema:
    """Configurable labels, relationship types, and property names."""
    ...
    def type_filter(self, var: str, value: str) -> str:
        return f"{var}.{self.entity_type_prop} = '{normalize_objective_type(value)}'"
    ...
    def df_perspective_predicate(self, df_var: str, entity_var: str, perspective_type: str) -> str:
        perspective_type = normalize_objective_type(perspective_type)
        return (
...
def normalize_objective_type(objective_type: str) -> str:
    """Use Segment terminology, while accepting Segment for backward compatibility."""
...
class CollaborationPatternCypher:
    """Factory for Cypher queries implementing collaboration patterns."""

    def __init__(self, schema: Schema = Schema()) -> None:
        self.s = schema

    # ------------------------------------------------------------------
    # Optional utility queries
    # ------------------------------------------------------------------

    def annotate_df_transition_time_numeric(self) -> str:
        """Annotate DF edges when Event.start and Event.end are numeric values."""
        s = self.s
        return f"""
MATCH {s.event('e1')}-[df:{s.df_rel}]->{s.event('e2')}
SET df.{s.df_transition_prop} = e2.{s.event_start_prop} - e1.{s.event_end_prop}
RETURN count(df) AS updatedDfEdges
""".strip()

    def annotate_df_transition_time_datetime(self) -> str:
        """Annotate DF edges when Event.start and Event.end are Neo4j datetime values."""
        s = self.s
        return f"""
MATCH {s.event('e1')}-[df:{s.df_rel}]->{s.event('e2')}
SET df.{s.df_transition_prop} = {s.seconds_between(f'e1.{s.event_end_prop}', f'e2.{s.event_start_prop}')}
RETURN count(df) AS updatedDfEdges
""".strip()

    def annotate_df_transition_time_datetime_string(self) -> str:
        """Annotate DF edges when Event.start and Event.end are ISO datetime strings."""
        s = self.s
        return f"""
MATCH {s.event('e1')}-[df:{s.df_rel}]->{s.event('e2')}
SET df.{s.df_transition_prop} = duration.inSeconds(
    datetime(e1.{s.event_end_prop}),
    datetime(e2.{s.event_start_prop})
).seconds
RETURN count(df) AS updatedDfEdges
""".strip()
...
WHERE {s.type_filter('o', objective_type)}
  AND {s.type_filter('ro_i', 'Robot')}
  AND {s.type_filter('ro_j', 'Robot')}
  AND {pred}
  AND ro_i <> ro_j
...
RETURN
  ...
  ro_j AS toRobot,
  transitionTime,
  e_i.{s.event_activity_prop} AS fromActivity,
  e_j.{s.event_activity_prop} AS toActivity
...
WITH
  o,
  collect(DISTINCT e) AS objectiveEvents,
...
WITH
  ...
  objectiveEnd,
  ro,
  count(DISTINCT e_by_robot) AS eventsByRobot
WITH
  o,
  objectiveEvents,
  objectiveStart,
  ...
  count(ro) AS teamSize,
  max(eventsByRobot) AS maxEventsPerRobot
WHERE teamSize > 1
RETURN
  o AS objective,
  team,
  teamSize,
  {objective_duration} AS objectiveDuration,
  toFloat(size(objectiveEvents)) / teamSize AS avgEventsPerRobot,
  maxEventsPerRobot,
  robotEventStats
...
""".strip()
    ...
    def capability_driven_return(self, objective_type: str = "Mission") -> str:
        ...
        Returns tuples: <e_i, e_j, e_k, objective, returningRobot,
        intermediateRobot, capabilities>.
        """
        s = self.s
        objective_type = normalize_objective_type(objective_type)
...
WHERE {s.type_filter('o', objective_type)}
  AND {s.type_filter('ro_a', 'Robot')}
  AND {s.type_filter('ro_b', 'Robot')}
  AND {pred1}
  AND {pred2}
  AND ro_a <> ro_b
...
RETURN
  ...
  e_k,
  o AS objective,
  ro_a AS returningRobot,
  ro_b AS intermediateRobot,
...
RETURN
  ...
  team2,
  sharedRobots,
  size(sharedRobots) AS robotCompetition,
  start1,
  end1,
  start2,
  end2,
  overlapStart,
  overlapEnd,
  {overlap_duration} AS overlapDuration
...
""".strip()
    ...
    def parallel_collaboration_segment(self) -> str:
        """Backward-compatible alias for parallel_collaboration_segment()."""
        return self.parallel_collaboration_segment()
...
RETURN
  ...
  downstreamEvent,
  latestEnd,
  downstreamEvent.{s.event_start_prop} AS downstreamStart,
  {sync_delay} AS syncDelay,
...
WITH
  ...
  count,
  avgSwitchTime,
  sum({event_duration}) AS activeTime
RETURN
  robot,
  count,
  avgSwitchTime,
...
WITH
  ...
  capReturnCount,
  avgReturnTime,
  count(DISTINCT ro) AS availability
RETURN
  capability,
  capReturnCount,
  availability,
